...
export function useChurchServices(isLocal?: boolean) {
    const [services, setServices] = useState<ChurchServiceFeed[]>([]);
        ...
        (async () => {
            try {
                ...
                if ((err as Error).name !== 'AbortError') {
                    setError('Unable to load service schedule.');
                }
            } finally {
                setLoading(false);
            }
...
/** "19:00:00", "19:40:00" → "7:00 PM - 7:40 PM" */
...
export function dayNameToNumber(day: string): number {
    return DAY_NAME_TO_NUMBER[day] ?? 0;
}